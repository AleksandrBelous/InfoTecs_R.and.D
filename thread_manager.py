...
class ThreadManager:
    ...
    def _ui_worker(self) -> None:
        """
        Русский:
            Рабочий поток для UI (обработка входящих сообщений)
            Аргументы: None
            Возвращаемое значение: None
    ...
    def send_message(self, message: str) -> bool:
        ...
        try:
            ...
            # Отправляем через сетевой менеджер
            if self.network.send_broadcast(outgoing_data):
                # Отображаем собственное сообщение в UI
                self.ui.display_message(nickname, message)
...
if __name__ == "__main__":
    ...
    class MockUI:
        def get_nickname(self): return "test_user"
        def display_message(self, nick, msg): pass
