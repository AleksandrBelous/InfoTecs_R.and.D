...
Примеры использования / Usage examples:
  ...
  python main.py --ip 192.168.1.100 --port 1234 --log
        ...
        parser.add_argument(
                '--ip',
                required=True,
                help='IPv4 адрес интерфейса / IPv4 interface address'
                )
        ...
        # Простая валидация IPv4 адреса
        try:
            if isinstance(args.ip, str) and args.ip.lower() in ['localhost', '127.0.0.1']:
                args.ip = '127.0.0.1'
            elif args.ip == '0.0.0.0':
                pass  # 0.0.0.0 разрешен для всех интерфейсов
            else:
